-- ========================================
-- AUTOMATED APPOINTMENT REMINDER SYSTEM V2 (FIXED)
-- Assumes appointments are confirmed unless cancelled
-- Includes business owner notifications
-- Fixed RLS policies for leads table structure
-- ========================================
...
-- Create policy for authenticated users to view their reminders (simplified)
CREATE POLICY "Users can view scheduled reminders"
ON scheduled_reminders
FOR SELECT
TO authenticated
USING (true); -- Allow all authenticated users to view reminders for now
...
BEGIN
  ...
  SELECT
    ...
    '99alecrodriguez@gmail.com'::TEXT as business_email, -- Replace with actual business email
...
-- Insert default business notification settings
...
DO $$
BEGIN
  RAISE NOTICE 'Automated reminder system V2 installed successfully!';
  RAISE NOTICE 'Next steps:';
  RAISE NOTICE '1. Update business_email in the process_due_reminders function';
  RAISE NOTICE '2. Set up cron job to call /api/cron/process-reminders every 10 minutes';
  RAISE NOTICE '3. Add CRON_SECRET to your environment variables';
END $$;
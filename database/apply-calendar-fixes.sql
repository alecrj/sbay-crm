-- Combined SQL to fix calendar auto-creation issues
-- Run this in your Supabase SQL Editor

-- First, setup RLS policies for calendar tables
...
DROP POLICY IF EXISTS "Enable read access for all users" ON property_calendars;
DROP POLICY IF EXISTS "Enable insert for authenticated users" ON property_calendars;
DROP POLICY IF EXISTS "Enable update for authenticated users" ON property_calendars;
DROP POLICY IF EXISTS "Enable delete for authenticated users" ON property_calendars;

DROP POLICY IF EXISTS "Enable read access for all users" ON calendar_availability;
DROP POLICY IF EXISTS "Enable insert for authenticated users" ON calendar_availability;
DROP POLICY IF EXISTS "Enable update for authenticated users" ON calendar_availability;
DROP POLICY IF EXISTS "Enable delete for authenticated users" ON calendar_availability;

DROP POLICY IF EXISTS "Enable read access for all users" ON calendar_blocked_dates;
DROP POLICY IF EXISTS "Enable insert for authenticated users" ON calendar_blocked_dates;
DROP POLICY IF EXISTS "Enable update for authenticated users" ON calendar_blocked_dates;
DROP POLICY IF EXISTS "Enable delete for authenticated users" ON calendar_blocked_dates;
...
-- Now create the database function to create property with calendar
-- This bypasses RLS issues by running as a privileged function

DROP FUNCTION IF EXISTS create_property_with_calendar(jsonb);
